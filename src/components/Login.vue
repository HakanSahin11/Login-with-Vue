    ...
    <md-content class="md-elevation-3">
        <div class="form">
          <md-field>
            ...
            <md-input v-model="login.cpr" autofocus></md-input>
          </md-field>
        ...
        <div class="actions md-layout md-alignment-center-space-between">
          <md-button class="md-raised md-primary" @click="sendToServer">Log In</md-button>
        </div>
    </md-content>
...
export default {
  ...
  data() {
   ...
   created(){
     axios.get("https://localhost:44319/api/main")
     .then((response) => {
  ...
  methods: {
    sendToServer() {
      if(this.loginTriesUsers.length < 3){
      axios.post('https://localhost:44319/api/main', this.login)
      .then(response => {
          if (response.data == "Success")
          {
            ...
            this.loginTries ++;
            // latest try
            this.loginTriesUsers.push(this.login.cpr)

            if (this.loginTriesUsers.filter(v => v === this.login.cpr).length >= 3 )
            {
              var loginBan = true;
              axios.post('https://localhost:44319/api/main', loginBan).then(response => {
                login.banTimer = response.data
              })
              debugger;
              alert(`Error! Too many login attempts detected! you have now been blocked from login until: ` + this.login.banTimer)
            }
            alert("Error! Wrong entered credentials deteched!")
          }
          else {
            alert("error")
          }
       ...
       alert(`Error! Your account have been locked until: ` + this.login.banTimer);
      }
    }
  },
}
